...
export async function POST(
    ...
    if (parsed.data.action === 'assignCandidate') {
        ...
        const session = await auth();
        try {
            await assignCandidate(params.cityId, parsed.data.candidateId, parsed.data.subjectId, session?.user?.id);
        } catch (e) {
    ...
    if (parsed.data.action === 'dismissCandidate') {
        try {
            await dismissCandidate(params.cityId, parsed.data.candidateId);
        } catch (e) {
